...
class TestSamplingMask(SamplingMaskTestMixin, CustomTestCase):
    ...
    def test_generate_returns_sampling_mask(self):
        ...
        # The sampled token comes from the captured support; it is never added
        # afterward to repair a reconstruction mismatch. The merged sampling
        # primitives retain ties at their threshold, so top-k can realize more
        # than k support entries.
    ...
    def test_sampling_mask_matches_topk_logprobs(self):
        ...
        in a successful mask. With ``temperature=1.0`` these are the sampler's
        distribution, so ``p = exp(logprob)`` are the exact probabilities. For
        each token, we check:
