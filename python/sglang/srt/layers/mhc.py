...
def get_mhc_pre_token_count_representatives(
    ...
    """Return one token count for each MHC prenorm split-count bucket."""
    reps = {}
...
def _prewarm_mhc_pre(
    ...
    buckets = get_mhc_pre_token_count_representatives(
        get_global_server_args().chunked_prefill_size, hc_mult * hidden_size
    )
    logger.info("GLM-5.3 MHC prenorm prewarm: %d split-count buckets", len(buckets))
