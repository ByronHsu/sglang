...
class Indexer(MultiPlatformOp):
    ...
    def __init__(
        ...
        if _use_dsa_indexer_fusion:
            cos_sin = self.rotary_emb.cos_sin_cache.to(torch.float32)
            half = cos_sin.shape[-1] // 2
            self._indexer_freqs_cis = torch.complex(
                cos_sin[:, :half].contiguous(), cos_sin[:, half:].contiguous()
            )
    ...
    def _fused_k_weights(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        key_and_weights, _ = self.wk_weights_proj(x)
        return key_and_weights.split([self.head_dim, self.n_heads], dim=-1)
    ...
    def _get_k_bf16(
        ...
        # Non-fusion path only; self.wk does not exist when fusion is enabled.
        key, _ = self.wk(x)
        key = self.k_norm(key)
        k_rope, _ = torch.split(
            key, [self.rope_head_dim, self.head_dim - self.rope_head_dim], dim=-1
    ...
    def _forward_cuda_k_only(
        ...
        return metadata.topk_transform(dummy_logits, self.index_topk), None
