...
def prepare_trtllm_nope_sparse_metadata_kernel(
    ...
    # TRTLLM-GEN returns NaNs for empty H512 sparse rows. Padding rows are not
    # consumed, so point them at a valid dummy token and attend to one element.
    is_empty = topk_len == 0
    tl.store(page_table_ptr + row * row_stride, 0, mask=is_empty)
    tl.store(topk_lens_ptr + row, tl.maximum(topk_len, 1))


def prepare_trtllm_nope_sparse_metadata(page_table: torch.Tensor) -> torch.Tensor:
