...
class Glm5NextForConditionalGenerationNextN(DeepseekV3ForCausalLMNextN):
    ...
    def _resolve_nextn_quant_config(self, config, quant_config):
        ...
        A quantized checkpoint can advertise a global FP8 method while listing
        its full ``model.layers.<num_hidden_layers>.*`` NextN block in
        ``quantization_config.ignore`` and storing it as BF16. Passing the target
        quant config into that block allocates FP8 parameters and loads the BF16
        payload without matching scales, corrupting the first QKV projection.
        """
